...
export default function Navbar() {
  ...
  // 🔴 ADDED: check if any service route is active
  ...
  return (
    <header className="fixed top-0 w-full z-50">
      ...
      <motion.div
        ...
        className="hidden md:flex overflow-hidden justify-around items-center bg-gray-900 text-white text-sm"
      >
        <span>📧 partha.basack@chhayaitservices.com</span>
        <span>📍 Taj Building, 30 SNP Area, Aambagan, Sakchi,
            East Singhbhum, Jharkhand, India – 831001</span>
            ...
            >
              <button
                className={`${baseLink} flex items-center ${
                  isServiceActive ? activeLink : ""
                }`} // 🔴 ACTIVE WHEN ANY SERVICE PAGE
              >
              ...
              <AnimatePresence>
                {servicesOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 12 }}
                    className="absolute top-full left-0 mt-5 bg-white rounded-xl shadow-lg px-6 py-4 min-w-[240px]"
                  >
                    {services.map((s) => (
                      <NavLink
                        key={s.name}
              ...
              <div className="flex-1 flex flex-col gap-5 px-6 pt-6 text-[16px]">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 ${
                    isServiceActive ? "text-blue-400" : "hover:text-blue-400"
                  }`} // 🔴 ACTIVE ON SERVICE PAGES
                >
